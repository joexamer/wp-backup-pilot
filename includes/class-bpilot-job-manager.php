...
class BPILOT_Job_Manager {
	const OPTION    = 'bpilot_jobs';
	const CRON_HOOK = 'bpilot_run_job';
	 ...
	 * Restore manager.
	 *
	 * @var BPILOT_Restore_Manager|null
	 */
	private $restore;
	 ...
	 * @param BPILOT_Restore_Manager|null $restore Restore manager.
	 */
	public function __construct( BPILOT_Backup_Manager $backups, BPILOT_Restore_Manager $restore = null ) {
	...
	private function advance_job( array $job ) {
		...
		if ( 'restore' === $job['type'] ) {
			if ( ! $this->restore ) {
				return new WP_Error( 'bpilot_restore_unavailable', __( 'Restore manager is unavailable.', 'backup-pilot' ) );
			}
	...
	private function cleanup_state( array $state ) {
		foreach ( array( 'work_dir', 'staging', 'preserve_temp' ) as $key ) {
			if ( ! empty( $state[ $key ] ) && is_dir( $state[ $key ] ) ) {
				BPILOT_Filesystem::delete_tree( $state[ $key ] );
			}
		}
	}
	...
	private function after_complete( array &$job ) {
		if ( 'backup' === $job['type'] && ! empty( $job['result']['path'] ) && file_exists( $job['result']['path'] ) ) {
			$remote = ( new BPILOT_Remote_Storage() )->maybe_upload( $job['result']['path'] );
		...
		$deleted = ( new BPILOT_Retention() )->apply();
		if ( $deleted > 0 ) {
