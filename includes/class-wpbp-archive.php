...
class WPBP_Archive {
	...
	public function create_zip( $source_dir, $zip_path ) {
		if ( ! class_exists( 'ZipArchive' ) ) {
			return new WP_Error( 'wpbp_zip_missing', __( 'The PHP ZipArchive extension is required.', 'backup-pilot-main' ) );
		}

		$zip = new ZipArchive();
		if ( true !== $zip->open( $zip_path, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
			return new WP_Error( 'wpbp_zip_open_failed', __( 'Could not create the backup ZIP file.', 'backup-pilot-main' ) );
		}
	...
	public function inspect( $zip_path ) {
		...
		if ( ! is_readable( $zip_path ) ) {
			return new WP_Error( 'wpbp_zip_unreadable', __( 'The backup package is not readable.', 'backup-pilot-main' ) );
		}

		$zip = new ZipArchive();
		if ( true !== $zip->open( $zip_path ) ) {
			return new WP_Error( 'wpbp_zip_invalid', __( 'The backup package could not be opened.', 'backup-pilot-main' ) );
		}
		$this->set_password( $zip );

		if ( false === $zip->locateName( 'manifest.json' ) ) {
			$zip->close();
			return new WP_Error( 'wpbp_zip_missing_entry', __( 'The package is missing manifest.json.', 'backup-pilot-main' ) );
		}
		...
		if ( ! is_array( $manifest ) ) {
			$zip->close();
			return new WP_Error( 'wpbp_manifest_invalid', __( 'The package manifest is invalid.', 'backup-pilot-main' ) );
		}
		...
		if ( ! empty( $includes['database'] ) && false === $zip->locateName( 'database.sql' ) ) {
			$zip->close();
			return new WP_Error( 'wpbp_zip_missing_entry', __( 'The package is missing database.sql.', 'backup-pilot-main' ) );
		}
		...
		if ( ! empty( $includes['files'] ) && ! $has_files ) {
			$zip->close();
			return new WP_Error( 'wpbp_zip_missing_files', __( 'The package does not contain a files directory.', 'backup-pilot-main' ) );
		}
	...
	public function add_zip_chunk( $zip_path, array $files, $offset, $limit = self::ZIP_CHUNK_COUNT ) {
		...
		if ( true !== $zip->open( $zip_path, $mode ) ) {
			return new WP_Error( 'wpbp_zip_open_failed', __( 'Could not open the backup ZIP file.', 'backup-pilot-main' ) );
		}
		$this->set_password( $zip );
	...
	public function extract_chunk( $zip_path, $destination, $offset, $limit = self::EXTRACT_CHUNK_COUNT ) {
		$zip = new ZipArchive();
		if ( true !== $zip->open( $zip_path ) ) {
			return new WP_Error( 'wpbp_zip_open_failed', __( 'Could not open the backup package.', 'backup-pilot-main' ) );
		}
		$this->set_password( $zip );
		...
		for ( $i = $offset; $i < $end; $i++ ) {
			...
			if ( false !== strpos( $name, '..' ) || 0 === strpos( $name, '/' ) || preg_match( '#^[A-Za-z]:#', $name ) ) {
				$zip->close();
				return new WP_Error( 'wpbp_zip_unsafe_path', __( 'The package contains an unsafe file path.', 'backup-pilot-main' ) );
			}
			$names[] = $name;
		}
		...
		if ( ! empty( $names ) && ! $zip->extractTo( $destination, $names ) ) {
			$zip->close();
			return new WP_Error( 'wpbp_zip_extract_failed', __( 'Could not extract the backup package.', 'backup-pilot-main' ) );
		}
	...
	private function set_password( ZipArchive $zip ) {
		$settings = class_exists( 'WPBP_Settings' ) ? WPBP_Settings::get() : array();
