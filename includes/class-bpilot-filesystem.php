...
class BPILOT_Filesystem {
	...
	/**
	 * Get the active WordPress content directory.
	 *
	 * @return string
	 */
	public static function content_dir() {
		return trailingslashit( dirname( get_theme_root() ) );
	}

	/**
	 * Get a managed wp-content section path.
	 *
	 * @param string $section Section name.
	 * @return string
	 */
	public static function content_section_path( $section ) {
		if ( 'uploads' === $section ) {
		...
		if ( 'plugins' === $section ) {
			return dirname( untrailingslashit( BPILOT_PLUGIN_DIR ) );
		}

		return trailingslashit( self::content_dir() ) . ltrim( $section, '/\\' );
	}

	/**
	 * Build a path relative to wp-content.
	 *
	 * @param string $path Absolute path.
	 * @return string
	 */
	public static function content_relative_path( $path ) {
		return ltrim( str_replace( wp_normalize_path( self::content_dir() ), '', wp_normalize_path( $path ) ), '/' );
	}
	 ...
	 * @return string
	 */
	public static function current_plugin_dir_name() {
		return basename( untrailingslashit( BPILOT_PLUGIN_DIR ) );
	}
	...
	public static function create_work_dir( $type ) {
		...
		return new WP_Error( 'bpilot_work_dir_failed', __( 'Could not create a temporary working directory.', 'backup-pilot' ) );
	}
