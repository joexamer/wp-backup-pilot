...
class BPILOT_Restore_Manager {
	 ...
	 * Constructor.
	 *
	 * @param BPILOT_Backup_Manager|null $backups Backup manager.
	 */
	public function __construct( BPILOT_Backup_Manager $backups = null ) {
		$this->backups = $backups ? $backups : new BPILOT_Backup_Manager();
	}
	...
	public function restore( $path, $rewrite_urls ) {
		...
		if ( is_wp_error( $safety_backup ) ) {
			return new WP_Error(
				'bpilot_pre_restore_backup_failed',
				sprintf(
		...
		try {
			...
			if ( is_wp_error( $extract ) ) {
				BPILOT_Filesystem::delete_tree( $staging );
				return $extract;
			}

			$sql_file = trailingslashit( $staging ) . 'database.sql';
			$db       = new BPILOT_Database();
			...
			if ( $rewrite_urls ) {
				$replacer = new BPILOT_Search_Replace();
			...
			BPILOT_Filesystem::delete_tree( $staging );
			return new WP_Error( 'bpilot_restore_failed', $exception->getMessage() );
		}
	}
	...
	public function process_chunk( array $state ) {
		...
		return new WP_Error( 'bpilot_restore_bad_phase', __( 'Restore job phase is invalid.', 'backup-pilot' ) );
	}
	...
	private function restore_files( $source_wp_content ) {
		if ( ! is_dir( $source_wp_content ) ) {
			return new WP_Error( 'bpilot_restore_files_missing', __( 'The package file contents are missing.', 'backup-pilot' ) );
		}

		$sections        = array( 'uploads', 'themes', 'plugins' );
		$restored        = array();
		$preserve_plugin = BPILOT_PLUGIN_DIR;
		$storage_paths   = BPILOT_Filesystem::paths();
		$preserve_store  = $storage_paths['base'];
		$preserve_temp   = trailingslashit( sys_get_temp_dir() ) . 'bpilot-preserve-' . wp_generate_password( 8, false, false );
		if ( ! wp_mkdir_p( $preserve_temp ) ) {
			return new WP_Error( 'bpilot_preserve_failed', __( 'Could not create a temporary plugin preservation directory.', 'backup-pilot' ) );
		}
		...
		if ( is_dir( $preserve_plugin ) ) {
			BPILOT_Filesystem::copy_tree( $preserve_plugin, $preserved_copy );
		}
		if ( is_dir( $preserve_store ) ) {
			BPILOT_Filesystem::copy_tree( $preserve_store, $preserved_store );
		}
		...
		foreach ( $sections as $section ) {
			...
			$target = BPILOT_Filesystem::content_section_path( $section );
			BPILOT_Filesystem::delete_tree( $target );
			wp_mkdir_p( $target );
			BPILOT_Filesystem::copy_tree( $source, $target );
			$restored[] = $section;
		}

		if ( is_dir( $preserved_copy ) ) {
			BPILOT_Filesystem::copy_tree( $preserved_copy, $preserve_plugin );
		}
		if ( is_dir( $preserved_store ) ) {
			BPILOT_Filesystem::copy_tree( $preserved_store, $preserve_store );
		}

		BPILOT_Filesystem::delete_tree( $preserve_temp );
		BPILOT_Filesystem::ensure_storage();
	...
	private function keep_plugin_active() {
		// WordPress.org guidelines do not allow plugins to change the activation
		// status of plugins programmatically. This method intentionally does nothing.
	}
	...
	private function process_verify_chunk( array $state ) {
		...
		if ( ! empty( $state['includes']['database'] ) && ! empty( $checksums['database_sql'] ) ) {
			...
			if ( ! is_readable( $sql_file ) || hash_file( 'sha256', $sql_file ) !== $checksums['database_sql'] ) {
				return new WP_Error( 'bpilot_checksum_database_failed', __( 'The database checksum does not match the package manifest.', 'backup-pilot' ) );
			}
		}
		...
		if ( ! empty( $checksums['files'] ) && is_array( $checksums['files'] ) ) {
			foreach ( $checksums['files'] as $relative => $hash ) {
				...
				if ( ! is_readable( $file ) || hash_file( 'sha256', $file ) !== $hash ) {
					...
					return new WP_Error( 'bpilot_checksum_file_failed', sprintf( __( 'A package file failed checksum validation: %s', 'backup-pilot' ), $relative ) );
				}
			}
		}
	...
	private function prepare_file_restore_chunk( array $state ) {
		...
		$preserve_plugin = BPILOT_PLUGIN_DIR;
		if ( is_dir( $preserve_plugin ) ) {
			BPILOT_Filesystem::copy_tree( $preserve_plugin, trailingslashit( $preserve_temp ) . 'backup-pilot' );
		}
		if ( is_dir( $storage_paths['base'] ) ) {
			BPILOT_Filesystem::copy_tree( $storage_paths['base'], trailingslashit( $preserve_temp ) . 'storage' );
		}
		...
		foreach ( array( 'uploads', 'themes', 'plugins' ) as $section ) {
			if ( is_dir( trailingslashit( $source_wp_content ) . $section ) ) {
				$sections[] = $section;
				BPILOT_Filesystem::delete_tree( BPILOT_Filesystem::content_section_path( $section ) );
				wp_mkdir_p( BPILOT_Filesystem::content_section_path( $section ) );
			}
		}
		...
		if ( false === file_put_contents( $list, wp_json_encode( $files ) ) ) {
			return new WP_Error( 'bpilot_restore_file_list_failed', __( 'Could not write restore file list.', 'backup-pilot' ) );
		}
	...
	private function process_file_restore_chunk( array $state ) {
		...
		if ( ! is_array( $files ) ) {
			return new WP_Error( 'bpilot_restore_file_list_missing', __( 'Restore file list could not be read.', 'backup-pilot' ) );
		}
	...
	private function process_cleanup_chunk( array $state ) {
		...
		if ( ! empty( $state['preserve_temp'] ) ) {
			if ( is_dir( trailingslashit( $state['preserve_temp'] ) . 'backup-pilot' ) ) {
				BPILOT_Filesystem::copy_tree( trailingslashit( $state['preserve_temp'] ) . 'backup-pilot', BPILOT_PLUGIN_DIR );
			}
			if ( is_dir( trailingslashit( $state['preserve_temp'] ) . 'storage' ) ) {
				BPILOT_Filesystem::copy_tree( trailingslashit( $state['preserve_temp'] ) . 'storage', $storage_paths['base'] );
			}
			BPILOT_Filesystem::delete_tree( $state['preserve_temp'] );
		}

		BPILOT_Filesystem::delete_tree( $state['staging'] );
		BPILOT_Filesystem::ensure_storage();
	...
	private function build_restore_file_list( $source_wp_content ) {
		...
		foreach ( $iterator as $item ) {
			...
			$files[]  = array(
				...
				'target' => trailingslashit( BPILOT_Filesystem::content_dir() ) . $relative,
			);
		}
	...
	private function external_work_dir( $prefix ) {
		$path = trailingslashit( sys_get_temp_dir() ) . 'bpilot-' . sanitize_key( $prefix ) . '-' . gmdate( 'Ymd-His' ) . '-' . wp_generate_password( 8, false, false );
		if ( ! wp_mkdir_p( $path ) ) {
			return new WP_Error( 'bpilot_temp_failed', __( 'Could not create an external temporary directory.', 'backup-pilot' ) );
		}
