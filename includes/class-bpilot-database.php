...
class BPILOT_Database {
	...
	public function start_export( $file ) {
		...
		if ( ! $handle ) {
			return new WP_Error( 'bpilot_sql_write_failed', __( 'Could not write the database export file.', 'backup-pilot' ) );
		}
	...
	public function export_table_chunk( $file, $table, $offset, $limit = 500 ) {
		...
		if ( 0 === (int) $offset ) {
			...
			if ( empty( $create[1] ) ) {
				...
				return new WP_Error( 'bpilot_sql_create_missing', sprintf( __( 'Could not read table structure for %s.', 'backup-pilot' ), $table ) );
			}
	...
	public function import( $file ) {
		...
		if ( ! is_readable( $file ) ) {
			return new WP_Error( 'bpilot_sql_unreadable', __( 'The database file is not readable.', 'backup-pilot' ) );
		}

		$handle = fopen( $file, 'rb' );
		if ( ! $handle ) {
			return new WP_Error( 'bpilot_sql_open_failed', __( 'Could not open the database file.', 'backup-pilot' ) );
		}
		...
		while ( ( $line = fgets( $handle ) ) !== false ) {
			...
			if ( ';' === substr( rtrim( $line ), -1 ) ) {
				...
				if ( false === $result ) {
					...
					return new WP_Error( 'bpilot_sql_query_failed', sprintf( __( 'Database import failed near: %s', 'backup-pilot' ), substr( trim( $statement ), 0, 120 ) ) );
				}
				$statement = '';
			}
		}
